...
export class BuildEngineFactory {
  ...
  private static buildEngine(config: Config, sheets: Sheets = {}, inputNamedExpressions: SerializedNamedExpression[] = [], stats: Statistics = config.useStats ? new Statistics() : new EmptyStatistics()): EngineState {
    ...
    const engine = this.assembleEngine(config, dependencyGraph, sheetMapping, lazilyTransformingAstService, functionRegistry, namedExpressions, stats, inputNamedExpressions)
    stats.measure(StatType.GRAPH_BUILD, () => {
      const graphBuilder = new GraphBuilder(dependencyGraph, engine.columnSearch, engine.parser, engine.cellContentParser, stats, new ArraySizePredictor(config, functionRegistry))
      graphBuilder.buildGraph(sheets, stats)
    })


    engine.evaluator.run()
  ...
  private static assembleEngine(config: Config, dependencyGraph: DependencyGraph, sheetMapping: SheetMapping, lazilyTransformingAstService: LazilyTransformingAstService, functionRegistry: FunctionRegistry, namedExpressions: NamedExpressions, stats: Statistics, inputNamedExpressions: SerializedNamedExpression[] = []) {
